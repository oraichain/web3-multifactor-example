...
function App() {
  ...
  const triggerLogin = async () => {
    ...
    try {
      ...
      const loginResponse = await (
        tKey.serviceProvider as OraiServiceProvider
      ).triggerLogin({
        typeOfLogin: "google",
        clientId:
      ...
      if (value.length > 10) {
        try {
          ...
          if (requiredShares <= 0) {
            ...
            setPrivateKey(reconstructedKey?.privKey.toString(16, 64));
            uiConsole(
              "Private Key: " + reconstructedKey.privKey.toString(16, 64),
            );
          }
  ...
  const requestNewShare = async () => {
    try {
      ...
      const currentEncPubKeyX = await (
        tKey.modules.shareTransfer as any
      ).requestNewShare(
        window.navigator.userAgent,
        tKey.getCurrentShareIndexes(),
      );
      console.log(
        "🚀 ~ file: Test.tsx:208 ~ requestNewShare ~ currentEncPubKeyX:",
        currentEncPubKeyX,
      );
      const shareStore = await (
        tKey.modules.shareTransfer as any
      ).startRequestStatusCheck(currentEncPubKeyX, true);
      const { privKey } = await tKey.reconstructKey(false);
      console.log(
        "🚀 ~ file: Test.tsx:208 ~ requestNewShare ~ privKey:",
        privKey,
      );
    } catch (error) {
